...
| **Video generation** | `seedance-2-0` (preferred premium default — cinematic, trailer, multi-shot, synced audio, lip-sync), `gemini-omni` (conversational video editing, reference tags, timecoded beats), `ai-video-gen`, `ltx2`, `comfyui-local` (local/offline via a user-run ComfyUI server, Apple Silicon-friendly) |
| **Audio** | `elevenlabs`, `music`, `sound-effects`, `acestep`, `runcomfy-music` (ACE-Step/ElevenLabs Music via RunComfy), `text-to-speech`, `azure-text-to-speech` (optional cloud TTS — tool `azure_tts`, same Speech key as `azure_stt`), `setup-api-key` |
