...
| 12 | **$0 + NVIDIA GPU** | Local video gen | WAN 2.1, Hunyuan, CogVideo, LTX — free, offline (CUDA only, not Mac) |
| 13 | **$0 + GPU** | Local Diffusion | Stable Diffusion images — free, offline |
| 14 | **varies by model** | RunComfy | Any ComfyUI-hosted model by `model_id` (FLUX, Kling, LTX, and community models) through one CLI — cloud, pay-per-run, despite the name |
| 15 | **$0 + GPU** | ComfyUI Local | Your own ComfyUI server + workflow — free, offline, works on Apple Silicon Macs |
...
| **Video Generation** | Grok, Kling Official, Kling via fal.ai, Runway, Veo, Gemini Omni, Higgsfield, MiniMax, HeyGen, RunComfy (any model_id) | WAN, Hunyuan, CogVideo, LTX (CUDA only), ComfyUI Local (Apple Silicon-friendly) | Pexels, Pixabay (stock) |
