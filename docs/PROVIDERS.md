...
| 14 | **$0 + NVIDIA GPU** | Local video gen | WAN 2.1, Hunyuan, CogVideo, LTX — free, offline (CUDA only, not Mac) |
| 15 | **$0 + GPU** | Local Diffusion | Stable Diffusion images — free, offline |
| 16 | **varies by model** | RunComfy | Any ComfyUI-hosted model by `model_id` (FLUX, Kling, LTX, and community models) through one CLI — cloud, pay-per-run, despite the name |
| 17 | **$0 + GPU** | ComfyUI Local | Your own ComfyUI server + workflow — free, offline, works on Apple Silicon Macs |
...
COMFYUI_API_URL=             # comfyui_local tool only — local ComfyUI server (Apple Silicon-friendly)
```
...
|-----------|----------------|-----------------|--------------|
| **Image Generation** | FLUX, Kling Official, Grok, Google Imagen, GPT Image 2, Recraft, RunComfy (any model_id) | Local Diffusion | Pexels, Pixabay (stock) |
| **Video Generation** | Grok, Kling Official, fal.ai, Seedance via Volcengine Ark, Runway, Veo, Gemini Omni, Higgsfield, MiniMax, HeyGen, Tencent Hunyuan, ComfyUI Partner Nodes, RunComfy (any model_id) | WAN, Hunyuan, CogVideo, LTX (CUDA only), ComfyUI WAN, ComfyUI MiniMax H3, ComfyUI Local (Apple Silicon-friendly) | Pexels, Pixabay (stock) |
...
| **Music Generation** | ElevenLabs, Suno, Google Lyria, RunComfy (ACE-Step 1.5, ElevenLabs Music) | — | ElevenLabs free tier |
